...
- [ ] **m20-01:** Add a presentation-only formatter and use it for follow-up row text, accessible labels, and edit defaults without changing the API/parser representation or resolved `linkedSource`. (Verification: `cd app && npm run lint --workspace web && npm run build --workspace web`)
