...
- [ ] **m15-02:** Boot-time YAML load as the sole provider source: parse/validate `SECOND_BRAIN_WEB_PROVIDERS_FILE`, refuse non-`v1:` keys and invalid entries, enforce exactly-one default (when non-empty), full-replace rebuild of `provider_profiles` with YAML keys as ids, tolerate missing/empty/directory path. Add the `yaml` package. (Verification: `cd app && npm test --workspace server -- provider-provisioning.test.ts` + full suite green)
