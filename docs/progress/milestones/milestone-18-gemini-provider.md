...
- [ ] **m18-01:** Add `gemini` to the app-to-SDK provider mapping and provider API allow-list, with regression tests proving Gemini profiles are accepted and translated unchanged for the SDK. (Verification: `cd app && npm test --workspace server -- agent-runner.test.ts provider-api.test.ts`)
