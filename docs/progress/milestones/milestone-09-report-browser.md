...
- [ ] **m09-01:** Scan `reports/YYYY/` for HTML, PDF, and Markdown metadata. (Verification: `npm test --workspace server -- reports-scan.test.ts`)
- [ ] **m09-02:** Serve report files through traversal-safe authenticated routes. (Verification: `npm test --workspace server -- reports-api.test.ts`)
- [ ] **m09-03:** Build the report shelf with year/type filters and title/path search. (Verification: `npm run lint --workspace web && npm run build --workspace web` and responsive visual verification)
