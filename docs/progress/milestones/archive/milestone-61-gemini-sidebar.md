# Milestone 61: Gemini-inspired sidebar

- [x] **m61-01** — Add a persistent collapsible desktop sidebar, icon-led New
  chat/Capture/Search chats actions, and title filtering for recent chats.
  Verification: `npm run lint --workspace web && npm run build --workspace web`.
- [x] **m61-02** — Move secondary pages into an icon-labelled bottom overflow
  menu beside Sign out, while retaining the mobile drawer and onboarding gates.
  Verification: `npm run lint --workspace web && npm run build --workspace web`.
- [x] **m61-03** — Run the full project verification gate and archive the
  milestone. Verification: `npm run lint && npm test && npm run build`.
