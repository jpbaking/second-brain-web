...
- [x] **m16-04:** Deliverable check: landing shows the last active chat; with an empty history it shows the new-chat state; sending from the new-chat state creates + streams a session and the sidebar updates; other screens render correctly inside the shell. (Verification: `cd app && npm run lint && npm test && npm run build` plus a headless-Chrome e2e of the above at 390 and 1280)
