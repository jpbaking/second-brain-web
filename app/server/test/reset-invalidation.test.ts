...
import { invalidateSessionsAndChallenges } from '../src/auth/reset.js'
...
describe('reset invalidation', () => {
  it('a session valid before reset no longer authenticates afterwards', async () => {
    ...
    // Reset invalidation revokes it.
    const result = invalidateSessionsAndChallenges(config.dataDir)
  ...
  it('drops pending challenges too', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'sbw-reset-'))
    scratch.push(root)
    const config = loadConfig({ SECOND_BRAIN_WEB_DATA_DIR: path.join(root, 'data') })
    prepareDatabases(config.dataDir)
    ...
    const result = invalidateSessionsAndChallenges(config.dataDir)
    expect(result.challengesDeleted).toBe(2)
  })
})
