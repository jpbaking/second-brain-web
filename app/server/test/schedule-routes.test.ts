...
describe('schedule routes', () => {
  it('requires authentication', async () => {
    const root = mkdtempSync(path.join(tmpdir(), 'sbw-sched-'))
    scratch.push(root)
    const config = loadConfig({ SECOND_BRAIN_WEB_DATA_DIR: path.join(root, 'data'), SECOND_BRAIN_WEB_SECRETS_KEY: 'k' })
    const app = buildApp(config)
    apps.push(app)

    expect((await app.inject({ method: 'GET', url: '/api/schedules' })).statusCode).toBe(401)
  })

  it('can create, list, and delete schedules', async () => {
    const root = mkdtempSync(path.join(tmpdir(), 'sbw-sched-'))
    scratch.push(root)
    const config = loadConfig({ SECOND_BRAIN_WEB_DATA_DIR: path.join(root, 'data'), SECOND_BRAIN_WEB_SECRETS_KEY: 'k' })
    ...
    const create = await app.inject({
      ...
      url: '/api/schedules',
      headers: { cookie },
      payload: { name: 'Morning Brief', workflow: 'brief', frequency: 'daily' }
    })
    expect(create.statusCode).toBe(201)
    const created = create.json()
    expect(created.name).toBe('Morning Brief')
    ...
    expect(list.json().schedules[0].id).toBe(created.id)

    // Delete schedule
    const del = await app.inject({ method: 'DELETE', url: `/api/schedules/${created.id}`, headers: { cookie } })
    expect(del.statusCode).toBe(200)

    // List again
    const list2 = await app.inject({ method: 'GET', url: '/api/schedules', headers: { cookie } })
    expect(list2.json().schedules).toHaveLength(0)
  })
})
