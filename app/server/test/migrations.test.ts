...
describe('database migrations', () => {
  it('migrates a fresh data dir from zero', () => {
    ...
    expect(tableExists(core, 'scheduled_jobs')).toBe(true)
    expect(tableExists(core, 'principal_profile')).toBe(true)
    expect(tableExists(core, 'report_provenance')).toBe(true)
    ...
    expect(schemaVersion(core)).toBe(15)
  ...
  it('rebuilds a deleted sidecar without touching core data', () => {
    ...
    expect(schemaVersion(rebuiltSidecar)).toBe(3)
    reopenedCore.close()
